...
def main():
    ...
    for pid, record in patient_db.items():
        ...
        response = call_agent(agent, query=f"Evaluate the claim for patient {pid}")
        print(response, "\n")
    ...
    # merged.to_csv("validation_comparison.csv", index=False, encoding="utf-8-sig")
    ...
    # pd.DataFrame(test_results).to_csv("submission.csv", index=False, encoding="utf-8-sig")
